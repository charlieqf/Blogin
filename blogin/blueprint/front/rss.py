...
def generate_rss(blogs):
    ...
    fg.link(href='https://digitaldwellings.tech/')
    fg.id(str(len(blogs)))
    for blog in blogs:
        fe = fg.add_entry()
        fe.title('[{}]'.format(blog.blog_types.name) + blog.title)
        fe.id(blog.id)
        fe.link(href='https://digitaldwellings.tech/blog/article/{}/'.format(blog.id))
        fe.description(blog.introduce)
        fe.guid(str(blog.id), permalink=False)  # Or: fe.guid(article.url, permalink=True)
        fe.author(name='Blogin', email='charlieqf@gmail.com')
